...
Jay Earley ja Howard Sturgis laajensivat Bratmanin kaaviota lis‰‰m‰ll‰ siihen ominaisuuksia ~\cite{Earley70}.
...
\caption{Earleyn ja Sturgisin T-kaavioelementit. Kuvan l‰hde ~\cite{Mogensen10}.}
\label{kuvaesimerkki}
\end{center}
\end{figure}
...
Koska k‰‰nt‰jien ohjelmointi matalan tason ohjelmointikielill‰ on eritt‰in vaivalloista ~\cite{Mogensen10},
...
yleisesti k‰ytetty menetelm‰ on ketjutus (bootstrapping) ~\cite{Mogensen10}. %IBID!

Ketjutus on tekniikka, jonka ydinajatus on k‰‰nt‰‰ jokin k‰‰nt‰j‰ sill‰ itsell‰‰n ~\cite{Mogensen10}
ja tavanomaisen k‰‰nt‰j‰n sanotaan olevan ketjutettu %suomennos?
...
osajoukon ~\cite{Mogensen10}. T‰m‰ ensimm‰isen vaiheen k‰‰nt‰j‰ voidaan ohjelmoida
...
sopivan osajoukon lˆyt‰minen ~\cite{Reynolds03}. Mik‰li ohjelmointikielt‰ laajennetaan vain
...
Ketjutus-menetelm‰‰ k‰ytet‰‰n usein myˆs yhdess‰ ristiink‰‰nt‰misen (cross compiling) kanssa ~\cite{Reynolds03}.
...
kehitys usealle ohjelmointikielelle ja/tai laitteistolle  on tehokkaampaa sek‰ kilpailukykyisemp‰‰ ~\cite{Guilan02}.
...
v‰likielt‰ (intermediate language) ~\cite{Speetjens76}. N‰in haluttu l‰hdekieli voidaan k‰‰nt‰‰
...
Kuva 7 demonstroi usein k‰ytetty‰ rakennetta ~\cite{Earley70}, jolla k‰‰nt‰j‰n jatkokehityst‰ voidaan
...
\begin{figure}[H]
	\ \newline
	\begin{center}
		\includegraphics[scale=1.0]{compiler-complex-color-T.png}
...
ketjuttamisen uuteen laitteistoon on mahdollista ilman lis‰ohjelmointia ~\cite{Earley70}.
...
voidaan suorittaa jo aiemmin k‰ytetyll‰ korkean tason ohjelmointikielell‰ ~\cite{Earley70}.
...
\section{Ristiink‰‰nt‰minen ja v‰likielet}

%https://www.gnu.org/savannah-checkouts/gnu/automake/manual/html_node/Cross_002dCompilation.html
Yleisesti tietokonej‰rjestelmiss‰ k‰ytett‰v‰t k‰‰nt‰j‰t tuottavat konekielt‰ samalle
laitteistolle, jossa niit‰ ajetaa. Ristiink‰‰nt‰misell‰ (cross compiling) tarkoitetaan
tilannetta, jossa kohdelaitteisto on jokin muu, kuin k‰‰nnˆst‰ suorittava laitteisto. %gcc
Ristiink‰‰nt‰misen er‰s t‰rke‰ sovellutus on k‰‰nt‰jien siirt‰minen laitteistosta toiseen ~\cite{intro}.
t‰h‰n jotain viel‰ v‰likielist‰
...
Oletetaan, ett‰ jollakin laitteistolla toimivia k‰‰nt‰ji‰ on n eri korkean tason ohjelmointikielelle
ja ohjelmia halutaan ristiink‰‰nt‰‰ kyseisill‰ kielill‰ m m‰‰r‰lle erilaisia laitteistoja.
Ilmeisin ratkaisu olisi kirjoitta k‰‰nt‰j‰ jokaiselle ohjelmointikieli-laitteisto-parille.
T‰llˆin tarvittavien k‰‰nt‰jien m‰‰r‰ on yht‰ suuri kuin ohjelmointikielten ja laitteistojen
karteesinen tulo, m*n.

\begin{figure}[H]
	\ \newline
	\begin{center}
		\includegraphics[scale=0.85]{UNCOL.png}
		\caption{Ristiink‰‰nt‰minen ilman v‰likieli‰ ja v‰likielten kanssa. L‰hde: ~\cite{Steel61}.}
		\label{kuvaesimerkki}
	\end{center}
\end{figure} 

%tuleeko t‰lle oma v‰liotsikko?
UNCOL (UNiversal Computer Oriented Language)  
Ei kehitt‰j‰‰ ~\cite{Strong58}.

Conwayn uncol SML ~\cite{Conway58}.
