...
Tekniikkaa voidaan käyttää usealla tavalla, useaan eri ongelmaan ~\cite{Earley70}. full, incremental ja cross!
...
kääntäjää kuvaava kaavio, jonka hän kehitti UNCOL-kaavion korvaajaksi ~\cite{Bratman61}.
Kaaviosta käy ilmi kääntäjän ymmärtämä kohde- ja lähdekieli sekä kieli, jolla kääntäjä toimii.
...
Jay Earley ja Howard Sturgis laajensivat Bratmanin kaaviota lisäämällä siihen ominaisuuksia ~\cite{Earley70}.
...
\caption{Earleyn ja Sturgisin T-kaavioelementit. Kuvan lähde ~\cite{Mogensen10}.}
\label{kuvaesimerkki}
\end{center}
\end{figure}
...
Esimerkkikuvassa kieli on D ~\cite{Mogensen10}.
...
Koska kääntäjien ohjelmointi matalan tason ohjelmointikielillä on erittäin vaivalloista ~\cite{Mogensen10},
...
yleisesti käytetty menetelmä on ketjutus (bootstrapping) ~\cite{Mogensen10}. %IBID!

Ketjutus on tekniikka, jonka ydinajatus on kääntää jokin kääntäjä sillä itsellään ~\cite{Mogensen10}
ja tavanomaisen kääntäjän sanotaan olevan ketjutettu %suomennos?
...
osajoukon ~\cite{Mogensen10}. Tämä ensimmäisen vaiheen kääntäjä voidaan ohjelmoida
...
suoritetaan useita kertoja siten, että kääntäjän tuntemaa ohjelmointikieltä kasvatetaan %tähän vois sanoa että mielenkiintoinen
ja siitä käännetään uusi kääntäjä, jonka avulla kieltä taas kasvatetaan, ja niin edelleen. %ongelma että mikä on tarpeeksi iso
Huomioitavaa prosessissa on se, että kääntäjän uusi versio on kehitettävä aina käyttäen %minimiksi. lähde tulostettu
...
Ketjutus-menetelmää käytetään usein myös yhdessä ristiinkääntämisen (cross compiling) kanssa ~\cite{Reynolds03}.
...
Vielä 1970-luvulla kääntäjät kirjoitettiin hyvin tarkasti tiettyä lähdekieltä ja laitteistoa varten ~\cite{Guilan02}.
1980-luvulla ruvettiin kehittämään enenevässä määrin kääntäjäkokonaisuuksia, jotka kykenevät 
kääntämään useita eri lähdekieliä. Tällaisissa systeemeissä kääntäjän sisäinen rakenne oli 
jaettu lähdekielestä riippuvaiseen etuosaan (frontend) sekä näiden jakamaan takaosaan (backend) ~\cite{Guilan02}. %ibid
...
kehitys usealle ohjelmointikielelle ja/tai laitteistolle  on tehokkaampaa sekä kilpailukykyisempää ~\cite{Guilan02}.
...
välikieltä (intermediate language) ~\cite{Speetjens76}. Näin haluttu lähdekieli voidaan kääntää
...
korkean tason ohjelmointikielellä kääntäjä lähdekieleltä välikielelle ~\cite{Earley70}.
...
Kuva 7 demonstroi usein käytettyä rakennetta ~\cite{Earley70}, jolla kääntäjän jatkokehitystä voidaan
...
\begin{figure}[H]
	\ \newline
	\begin{center}
		\includegraphics[scale=0.75]{compiler-complex-color-T.png}
...
ketjuttamisen uuteen laitteistoon on mahdollista ilman lisäohjelmointia ~\cite{Earley70}.
...
voidaan suorittaa jo aiemmin käytetyllä korkean tason ohjelmointikielellä ~\cite{Earley70}.
...
%\subsection{ristiinkääntäminen ja välikielet}
