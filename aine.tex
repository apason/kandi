...
suoritetaan useita kertoja siten, ett‰ k‰‰nt‰j‰n tuntemaa ohjelmointikielt‰ kasvatetaan %t‰h‰n vois sanoa ett‰ mielenkiintoinen
ja siit‰ k‰‰nnet‰‰n uusi k‰‰nt‰j‰ jonka avulla kielt‰ taas kasvatetaan ja niin edelleen.%ongelma ett‰ mik‰ on tarpeeksi iso
Huomioitavaa prosessissa on se, ett‰ k‰‰nt‰j‰n uusi versio on kehitett‰v‰ aina k‰ytt‰en %minimiksi. l‰hde tulostettu
...
Oletetaan ett‰ laitteistolle, joka ymm‰rt‰‰ konekielt‰ ML1 on toteutettu kielen PL k‰‰nt‰j‰.
Haluttaessa kielen PL k‰‰nt‰j‰ toimimaan sek‰ tuottamaan uuden laitteiston kohdekoodia ML2, 
voidaan kielen PL k‰‰nt‰j‰ ohjelmoida uudelle laitteistolle ristiink‰‰nt‰mist‰ ja bootstrapingi‰ k‰ytt‰en
seuraavalla tavalla: Ensin kielell‰ PL ohjelmoidaan kielen PL k‰‰nt‰j‰ uudelle laitteistolle.
...
kehitys usealle ohjelmointikielelle ja/tai laitteistolle  on tehokkaampaa sek‰ kilpailukykyisemp‰‰. %LƒHDE!
...
Oletettaen, ett‰ jollekin v‰likielelle ja jollekin korkean tason ohjelmointikielelle on
...
helpottaa huomattavasti. Uuden ohjelmointikielen toteuttaminen kyseisen k‰‰nt‰j‰n avulla on mahdollista
korvaamalla toisen vaiheen ensimm‰inen k‰‰nt‰j‰ (punaisella) siten, ett‰ uusi l‰hdekieli (SL2)
kyet‰‰n k‰‰nt‰m‰‰n v‰likielelle (IL) samalla kielell‰, jolla k‰‰nt‰j‰kompleksi on
suurimmaksi osaksi kirjoitettu (CWL). Esimerkiksi haluttaessa kyseisell‰ j‰rjestelm‰ll‰
...
ett‰ itsens‰ bootstr‰pp‰‰misen uudelle laitteistolle. Jos k‰‰nt‰j‰kokonaisuuden halutaan
k‰‰nt‰v‰n ymm‰rt‰m‰‰ns‰ l‰hdekielt‰ uudelle laitteistolle, jonka konekieli on ML2, joudutaan siihen
edelleen kirjoittamaan vain yksi uusi osa. T‰m‰ osa kirjoitetaan kielell‰ CWL ja
se k‰‰nt‰‰ v‰likielt‰ IL uuden laitteiston ymm‰rt‰m‰lle konekielelle ML2.  Kun kyseinen
osa k‰‰nnte‰‰n jo olemassa olevalla k‰‰nt‰j‰ll‰, saadaan tulokseksi
samassa laitteistossa toimiva k‰‰nt‰j‰ v‰likielelt‰ uudelle konekielelle ML2.
...
bootstr‰pp‰‰minen uuteen laitteistoon on mahdollista ilman lis‰ohjelmointia ~\cite{Earley70}.
...
\begin{figure}[H]
	...
	\begin{center}
		...
		\caption{K‰‰n‰t‰kokonaisuus bootstr‰p.}
		\label{kuvaesimerkki}
	\end{center}
\end{figure}

Toisessa ja kolmannessa vaiheessa saadut k‰‰nt‰j‰t (sininen) sijoittamalla alkuper‰iseen
...
siirt‰misen uuteen laitteistoon voidaan saavuttaa
vain pienen osan uudelleenohjelmoinnilla. Lis‰ksi uudelleenohjelmointi
voidaan suorittaa jo aiemmin k‰ytetyll‰ korkean tason ohjelmointikielell‰ CWL:ll‰ ~\cite{Earley70}.
