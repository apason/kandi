...
sekä helpompaa. Niiden käyttäminen tekee ohjelmakoodista myös ymmärrettävämpää ja
...
ovat ymmärrettävämpiä kuin vastaava symbolisella konekielellä ohjelmoituna.
...
helpottavia työkaluja, tuli ne ohjelmoida suoraan konekoodilla
%% Teoriassa
...
konekieli silti lähes poikkeuksetta parempaa. Tämän takia symbolisia
konekieliä käytetään vielä nykyisin matalan tason ohjelmoinnin lisäksi
...
tuottaa tästä linkkerin avulla konekielisen suoritettavan ohjelman.
Symboliset konekielet ovat matalan tason laiteriippuvaisia ohjelmointikieliä
joille on tyypillistä lähes täysi koodin symbolisuus. Tämä tarkoittaa sitä,
että suurin osa ohjelmakoodista on käännettävissä yksi yhteen laitteiston
ymmärtämän konekielen kanssa. Poikkeuksen tekee kuitenkin ohjelman osoitteina
käytettävät tunnisteet (label), joiden arvot assemblykääntäjä voi
vapaasti päättää. Tunnisteina voi toimia joko paikat ohjelman koodiosassa
tai muuttujina käytetyt muistipaikat. Symbolisen konekielen avainsanat
ovat siis symboleja laitteiston ymmärtämälle binäärille. Konekielellä on
...
kirjoitusvirheiden määrää ja tekee koodista helpomman kirjoittaa ja käydä läpi.
...
Kuva 1 selventää symbolisten konekielten ennalta määrättyjen symbolien
...
Kaikki harmaalla pohjalla oleva koodi käännetään siis yksi yhteen.
...
Korkena tason ohjelmointikielellä tarkoiteteen ohjelmointikieltä, % pitäiskö sanoa että tarkoitetaan tässä dokumentissa? ei lähdettä?
jossa kielen sekä siitä käännettävän konekielen välillä  % nyt vittu poissuljetaan ne kääntäjät jotka ei käännä assyks tai binääriks!
on selkeä abstraktiotaso ja kääntäminen edellyttää muutakin, kuin mekaanista
sanojen vaihtamista ennalta määrättyjen sääntöjen perusteella. Tämän määritelmän
perusteella korkean tason ohjelmointikielillä tarkoitetaan niitä kieliä,
...
myös matemaattinen esitystapa, jolla sen toimintaa voitiin täsmällisesti
kuvailla. Vaikka esitystapa oli vaikea eikä kyseisiä Turingin
esittelemiä laitteita ollut kuin teoreettisella tasolla,
Turingin esitystapa edusti kehittyneintä formaalia
kuvausta, 'kieltä', mitä siihen aikaan oli olemassa.

Toisen maailmansodan jälkeen vuonna 1945 saksalainen Konrad Zuse aloitti 
...
mille tahansa laskentaongelmalle. Tässä hän onnistuikin varsin hyvin.
...
Von Neumann ja Goldstine esittivät ratkaisuksi virtausdiagrammia (flow diagram),
esitystapaa jossa ohjelmat kuvataan nuolien ja laatikoiden avulla.
...
olivat ne kaikki merkittäviä askeleita kohti parempia ohjelmointikieliä sekä
...
Sitä kehitti John W. Mauchly vuonna 1949 ja William F. Schmitt toteutti sille tulkin.
...
Short Code oli siis algebrallinen tulkki, joka osasi suorittaa aritmeettisia
...
siinä kuvattiin ensimmäistä kertaa menetelmä kääntäjien toteuttamisesta
...
teorian kehityksen kannalta se oli kuitenkin korvaamaton. Böhmin kääntäjä
...
operaattoreiden sidontajärjestyksen, sekä osasi käsitellä sulkeita aritmeettisissa
...
käyttökelpoisten ohjelmien tekemiseen.
...
Mathematical FORmula TRANstating system' --- FORTRAN. Kuten jo aiemmin oli todettu,
...
% fortran oli ensimmäiojnen kieli jossa muuttujan nimi voi olla yli merkin pituinen
...
kohtuullisen tehokasta koodia ja keittäjät sanoivat sen olevan lähes
...
voivat olla useamman merkin pituisia.

Ensimmäisen julkaisun jälkeen FORTRANissa oli kuitenkin useita ognelmia.
Bugeja oli paljon ja eräs FORTRANIN kehittäjistä, Saul Rosen, %OLIKO SAUL ROSEN kehittäjä!?
...
and get a new new executable thet both uses and generates the new parameter passing style ~\cite{Appel}.
...
joita kääntäjät suorittavat. Oletetaan että käytössä on c-kielellä kirjoitettu
...
kääntäjä.
Huomattavaa on, että prosessin alimman tasoinen kääntäjä toimii aina jossakin todellisessa
...
Alkuperäisen T-kaavion kuvatessa vain kääntäjiä, Earleyn ja Sturgisin kaavioissa pysyi
kuvaamaan myös tulkkeja. Lisäksi kaavioon sisällytettiin suoritusta kuvaava osa
sekä sovellusohjelmaa kuvaava kaavio. Heidän kaavioissaan määritellään seuraavan kuvan
mukaiset elementit.
...
\ \newline
\begin{center}
\includegraphics[scale=0.5]{notations-T.png}
\caption{Earleyn ja Sturgisin versio T-kaavioista}
\label{kuvaesimerkki}
\end{center}
\end{figure}

Vasemmalta oikealle käytynä ensimmäinen kuva on Bratmanin alkuperäisin T-kaavion vastaava elementti.
...
siis yksittäistä käännöstä, jossa A-kielinen ohjelma käännetään B kieliseksi ohjelmaksi.
...
\subsection{bootstrapping} %laitetaanko tähän kappaleeseen bootstrapping + cross compiling?
...
Tekniikkaa voidaan käyttää usealla tavalla, useaan eri ongelmaan ~\cite{Earley70}. %ibid
%full bootstrap
...
\begin{table}[h!]
  ...
  \begin{tabular}{| l | l | l |}
    ...
    Plankalkül & Zuse & Ohjelmointikieli, Hierarkinen data \\ \hline
