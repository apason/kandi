...
\title{K‰‰nt‰j‰tekniikan historia ja kehittyminen}
...
%TƒN LAUSEEN vois ottaa vittuun?
...
Korkean tason ohjelmointikielten sek‰ symbolisten konekielten
k‰yttˆ helpottavat ja nopeuttavat ohjelmointia.
...
tuli suosittua. Ristiink‰‰nt‰misen helpottamiseksi kehitettiin
v‰likieli‰. Niiden avulla tarvittavan ohjelmoinnin m‰‰r‰ saatiin pienemm‰ksi~\cite[15]{Speetjens76}.
...
laitteistojen v‰lill‰~\cite[610]{Earley70}.
...
Useissa assemblyk‰‰nt‰jiss‰ onkin direktiivi, jolla voidaan varata muistia ja viitata
...
jossa l‰hdekieli ja siit‰ k‰‰nnett‰v‰ kohdekieli ovat 
...
t‰sm‰llinen tapa algoritmien kuvaamiseksi oli niiden kirjoittaminen konekielill‰.
...
se edusti Alonzo Churchin lambdakalkyylin~\cite[]{Church36} ohella kehittyneint‰ formaalia
laskennan kuvausta, joka siihen aikaan oli olemassa.
...
sill‰ artikkeli julkaistiin vasta vuonna 1972.

%LOHKOKAAVIO
Samoihin aikoihin Zusen kanssa myˆs Yhdysvaltalaiset Herman Goldstine ja
John von Neumann kehittiv‰t laskennan formalismia. Heid‰n ratkaisunsa
...
%CURRYN PASKA
Vuonna 1946 Marylandissa tyˆskennellyt Haskell B. Curry kehitti ENIAC:lle
...
k‰‰nt‰miseen. Curryn formalismi oli kuitenkin hieman luonnoton,
sill‰ suoritusyksikˆill‰ oli useita lopetuskohtia sek‰ nykykielist‰ poiketen
useita aloituskohtia~\cite[21]{Knuth76}.
Historiallisesti tyˆ oli kuitenkin merkitt‰v‰, sill‰ Curry kuvaili algoritmeja
joilla h‰nen kehitt‰m‰st‰‰n ohjelmointikielest‰  pystyttiin tuottamaan konekielt‰. N‰it‰ rekursiivisia,
vaikkakin toteuttamatta j‰‰neit‰, algoritmeja voidaankin pit‰‰ ensimm‰isin‰
koodin generointiin tarkoitettuina algoritmeina~\cite[22]{Knuth76}.
Curryn artikkeleita koodin generoinnista ei kuitenkaan ikin‰
julkaistu, joten niiden vaikutus ohjelmointikielten kehitykseen
j‰i v‰h‰iseksi~\cite[20]{Knuth76}.
...
niiden k‰‰nt‰ji‰. Ilman t‰sm‰llisi‰ esitystapoja ei varsinainen k‰‰nnˆksen tai
tulkkauksen suorittaminen ikin‰ olisi voinut tulla mahdolliseksi.
...
manuaalissa kerrotaan kuitenkin kuinka ohjelmaa voidaan k‰ytt‰‰ (ibid).
Short Code oli siis tulkki, joka osasi suorittaa aritmeettisia
...
% VITTU MITƒ PASKAA
Vaikka Rutishauser ja Bˆhm olivat kumpikin ohjelmoineet
omat k‰‰nt‰j‰ns‰, pidet‰‰n ensimm‰isen‰ oikeana k‰‰nt‰j‰n‰ silti
Alick E. Glennien 1952 valmistamaa AUTOCODE-ohjelmistoa~\cite[42]{Knuth76}. Rutishauserin ja Bˆhmin k‰‰nt‰jist‰ poiketen
AUTOCODE ei toiminut hypoteettisella laitteistolla joten sen tuottama konekieli
oli t‰ten suoritettavissa ~\cite[45]{Knuth76}. AUTOCODE-kielt‰ pystyttiin siis k‰ytt‰m‰‰n oikeiden,
...
automaattisen ohjelmoinnin j‰rjestelm‰‰. Sen oli tarkoitus olla hyvin
...
puolet molemmista~\cite[1]{IBM54}. 
Kyseinen FORTRAN-dokumentti sis‰lt‰‰ myˆs ensimm‰isen yrityksen esitt‰‰ ohjelmointikielen
kielioppi t‰sm‰llisesti. T‰t‰ voidaan pit‰‰ Backuksen myˆhemmin esitt‰m‰n kielioppimuodon
Backus Naur Formin (BNF) edelt‰j‰n‰~\cite[63]{Knuth76}.

Kun FORTRAN kaksi ja puoli vuotta myˆhemmin saatiin toteutettua, oli se
aikansa tehokkain sek‰ monipuolisin ohjelmointikieli. FORTRAN:n k‰‰nt‰j‰ll‰ tuotettiin
...
FORTRAN:ssa oli myˆs paljon ominaisuuksia, joita ei oltu aiemmin n‰hty.
Se oli esimerkiksi ensimm‰inen ohjelmointikieli, jossa muuttujien nimet
voivat olla useamman merkin pituisia~\cite[62]{Knuth76}.

Ensimm‰isen julkaisun j‰lkeen FORTRAN:ssa oli kuitenkin useita ongelmia.
Virheit‰ oli paljon ja moni ep‰ili
ettei se ikin‰ tulisi toimimaan~\cite[4]{Rosen64}.
Vaikeuksista huolimatta FORTRAN:sta tuli hyvin suosittu ja sit‰ k‰ytettiin
enemm‰n kuin oltiin osattu odottaa.

Taulukko 1 tiivist‰‰ aiemmissa kappaleissa esitettyjen ohjelmointikielten merkitt‰vimm‰t
piirteet. Lis‰ksi kerrotaan ohjelmointikielten nimet sek‰ p‰‰tekij‰t.

\begin{table}[h!]
  \centering
  \caption{Yhteenvetotaulukko~\cite[93]{Knuth76}.} 
  \label{tab:table1}
  \begin{tabular}{| l | l | l |}
    \hline
    ...
    Lohkokaaviot & Goldstine, & Hyv‰ksytty ohjelmointimenetelm‰ \\
     & Von Neumann &  \\ \hline
...
Ongelmana edell‰ kuvatussa menetelm‰ss‰ on suuri sek‰ kasvava joukko l‰hde- ja
...
H‰n kehitti sen
\emph{The Problem of Programming Communication with Changing Machines: A Proposed Solution}-julkaisussa
esitetyn UNCOL-kaavion korvaajaksi~\cite[]{Bratman61}. 


\subsection{Alkuper‰inen Bratman-kaavio}
Harvey Bratmanin esitt‰m‰ kaavio kuvaa yksitt‰ist‰ k‰‰nt‰j‰‰. 
...
jonka kohdekieli on x86 (punainen). Kahden ensimm‰isen k‰‰nt‰j‰n yhteistyˆll‰ saadaan siis kolmas
...
\subsection{Earleyn ja Sturgisin merkint‰tapa}
...
toisiinsa. Kuvan esimerkin k‰‰nnˆs ei voi ottaa l‰hdeohjelmakseen muuta kuin C++:lla kirjoitetun ohjelman.
...
\begin{figure}[H]
	...
	\begin{center}
		...
		\caption{Iteratiivinen ketjutus. \label{iterative-bootstrap}}
		\label{kuvaesimerkki}
	\end{center}
\end{figure}
...
kehitys usealle ohjelmointikielelle tai laitteistolle  on tehokkaampaa sek‰ kilpailukykyisemp‰‰ (ibid).
...
helpottaa huomattavasti. Ketjutuksen vaiheet on merkattu kuvaan sulkeiden sis‰‰n.
...
komponentit. Ainoa komponentti, jota ei saada suoraan sijoittamalla, on
alkuper‰isen k‰‰nt‰j‰n ensimm‰isen vaiheen toinen komponentti.
